# Agile Rgb MSI Builder for Windows
# Adapted from the OpenRGB build-msi.sh / build-msi.ps1 scripts
...
# Configuration
$PRODUCTNAME        = "AgileRgb"
$VENDOR             = $PRODUCTNAME

$TLD                = "com"
$WEBSITE            = "https://github.com/JarzDev/agile-rgb"
...
$GITURL             = "https://github.com/JarzDev/agile-rgb/-"
...
$BUILT_PATH         = "Agile Rgb Windows 64-bit\*"
$WORKING_PATH       = "Agile Rgb Windows MSI Installer\"
...
$XML_FILE += "            Comments=`"$PRODUCTCOMMENT`" Manufacturer='$VENDOR'`r`n"
...
$XML_FILE += "        <Icon Id='AgileRgbIcon' SourceFile='$ICONFILE'/>`r`n"
$XML_FILE += "        <Property Id='ARPPRODUCTICON' Value='AgileRgbIcon'/>`r`n"
$XML_FILE += "        <Property Id='ARPURLINFOABOUT' Value='$WEBSITE'/>`r`n"
...
$XML_FILE += "                <Control Id='MainText' Type='Text' X='40' Y='60' Width='300' Height='40' Text='$PRODUCTNAME uses the PawnIO driver to control RGB lighting on DRAM modules and some older motherboards.  For these devices to work, you must manually install PawnIO by visiting the link below.' />`r`n"
...
$XML_FILE += "                <Control Id='ThirdText' Type='Text' X='40' Y='180' Width='300' Height='40' Text='With PawnIO installed, $PRODUCTNAME must be run as administrator to access these devices.' />`r`n"
$XML_FILE += "            </Dialog>`r`n"
$XML_FILE += "`r`n"
...
$XML_FILE += "                <Control Id='BottomLine' Type='Line' X='0' Y='234' Width='373' Height='0' />`r`n"
$XML_FILE += "`r`n"
$XML_FILE += "                <Control Id='TypicalText' Type='Text' X='40' Y='60' Width='300' Height='40' Text='Installing $PRODUCTNAME without registering as a system service means nothing runs automatically in the background by default.  Accessing devices through PawnIO will require running $PRODUCTNAME as administrator.' />`r`n"
$XML_FILE += "                <Control Id='TypicalButton' Type='PushButton' X='40' Y='100' Width='200' Height='17' ToolTip='Does not register $PRODUCTNAME as a system service.  Accessing certain devices will require running the application as administrator.' Default='yes' Text='Do not Install System Service (Default)'>`r`n"
...
$XML_FILE += "                </Control>`r`n"
$XML_FILE += "                <Control Id='CompleteText' Type='Text' X='40' Y='120' Width='300' Height='40' Text='Installing $PRODUCTNAME as a system service means the application will run automatically in the background.  Accessing devices through PawnIO will not require running $PRODUCTNAME as administrator.' />`r`n"
$XML_FILE += "                <Control Id='CompleteWarning' Type='Text' X='40' Y='160' Width='300' Height='40' Text='Warning: Installing $PRODUCTNAME as a system service is experimental in the current release.  Settings do not synchronize between the user interface and the service.  If you encounter issues with the service, you can uninstall it and use the non-service installation instead.' />`r`n"
$XML_FILE += "                <Control Id='CompleteButton' Type='PushButton' X='40' Y='210' Width='200' Height='17' ToolTip='Registers $PRODUCTNAME system service, allowing access to devices which require administrator access without having to run the application as administrator.' Text='Install System Service'>`r`n"
...
$XML_FILE += $SVC_FILE
$XML_FILE += "                        <ServiceInstall Id='ServiceInstaller' Type='ownProcess' Name='$PRODUCTNAME' DisplayName='$PRODUCTNAME' Description='$PRODUCTNAME SDK Server' Start='auto' Account='[SERVICEACCOUNT]' Password='[SERVICEPASSWORD]' ErrorControl='normal'/>`r`n"
$XML_FILE += "                        <ServiceControl Id='StartService' Start='install' Stop='both' Remove='uninstall' Name='$PRODUCTNAME' Wait='yes'/>`r`n"
$XML_FILE += "                    </Component>`r`n"
